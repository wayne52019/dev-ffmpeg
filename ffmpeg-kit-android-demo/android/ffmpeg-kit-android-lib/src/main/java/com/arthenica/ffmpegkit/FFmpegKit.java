...
public class FFmpegKit {
    ...
    /**
    *給Unity 影片轉檔用的接口
     * @param command            FFmpeg 指令
     * @param completeGameObjectName   完成callBack的Unity物件名稱
     * @param completeFunctionName     完成callBack的Unity函式名稱
     * @param statisticsGameObject   狀態callBack的Unity物件名稱
     * @param statisticsGameFunctionName     狀態callBack的Unity函式名稱
     */
    public static FFmpegSession executeAsync(final String command,
                                             final String duration,
                                             final String completeGameObjectName,
                                             final String completeFunctionName,
                                             final String statisticsGameObject,
                                             final String statisticsGameFunctionName) {
        //設定轉檔完成的回調
        FFmpegSessionCompleteCallback completeCallback = new FFmpegSessionCompleteCallback() {
            @Override
            public void apply(FFmpegSession session) {
                Android2Unity.callUnity(completeGameObjectName, completeFunctionName, session.returnCode.toString());
            }
        };
        //影片轉檔的進度回調
        StatisticsCallback statisticsCallback = new StatisticsCallback() {
            ...
            public void apply(Statistics statistics) {
                Log.d("FFmpegKit Time", "getTime: " + statistics.getTime() + " duration:" + duration.toString());
                int progress = (int) ((float) statistics.getTime() / Float.parseFloat(duration) * 100);
                Android2Unity.callUnity(statisticsGameObject, statisticsGameFunctionName, String.valueOf(progress));
            }
        };

        return executeWithArgumentsAsync(FFmpegKitConfig.parseArguments(command), completeCallback, null, statisticsCallback);
    }
